...
#define imguigml_set_item_default_focus
...
#define imguigml_add_font_from_ttf
...
///@param {Real} _font_size
///@returns _font_index - use the font_index with imguigml_push_font()
var _font_data = [ argument[0], argument_count > 1 ? argument[1] : 10.0 ];
if (!__imguigml_ext_call(_extImguiGML_reserve_font_index()))
	return;

with (imgui) {
	sr_array_push_back(Deferred_fonts, _font_data);
}
...
var _buff = __imguigml_wrapper_buffer();
sr_buffer_write_val(_buff, _font_index);
...
Deferred_fonts = sr_array_create();
...
while (sr_array_size(Deferred_fonts) > 0) {
	var _font_data = sr_array_pop_back(Deferred_fonts);	

	var buff = __imguigml_wrapper_buffer();
	sr_buffer_write_val(buff, _font_data[0], ERousrData.String);
	sr_buffer_write_val(buff, _font_data[1], ERousrData.Float);
	if (__imguigml_ext_call(_extImguiGML_add_font_from_ttf()))
	{
		var out = __Imgui_out;
		var ret = buffer_read(out, buffer_s8) != 0;
		if (ret) {
...
#define imguigml_begin_tab_item
